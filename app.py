...
def parse_ct_file(self, data, i, j):
    reset_flag = False
    try:
...
class GC_range_picker(QMainWindow):
    ...
    def on_change_1(self):
        ...
        try:
            ...
            error.showMessage(
                f"Please make sure to give a value lower than the chosen maximum"
            )
    ...
    def on_change_2(self):
        ...
        try:
            ...
            error = QErrorMessage(self)
            error.showMessage(
                f"Please make sure to provide a value greater than the chosen minimum"
            )
...
class Loop_input_widget(QMainWindow):
    ...
    def on_change_2(self):
        ...
        try:
            ...
            error = QErrorMessage(self)
            error.showMessage(
                f"Please make sure to give a value greater than the chosen minimum"
            )
    ...
    def save_configuration(self):
        ...
        if str(filename[0]) != "":
            ...
            self.parent().input_sequence = {
                self.parent().header[_]: data[self.parent().header[_]]
                ...
                if len(text) != self.population_size[_]:
                    ...
                    if self.header[_].islower():
                        for i in range(len(self.strand)):
                            if data[self.header[_].lower()] in self.strand[i].bases:
                            ...
                            if self.header[_].upper() in data.keys():
                                if data[self.header[_].upper()] in self.strand[i].bases:
                                    old_base = self.strand[i].bases
                                    updated_base = old_base.replace(
                    ...
                    if self.header[_].isupper():
                        for j in range(len(self.strand)):
                            if data[self.header[_].lower()] in self.strand[j].bases:
                            ...
                            if self.header[_].upper() in data.keys():
                                if data[self.header[_].upper()] in self.strand[j].bases:
                                    old_base = self.strand[j].bases
                                    updated_base = old_base.replace(
                ...
                if (
                    ...
                    with open(f"{i}_{j}.ct", "r") as configfile:
                        ...
                        for line in configfile:
                            try:
                                if counter > (
                                    len(self.strand[i].bases)
                                    + len(self.strand[j].bases)
                                    + 4
                                ):
                                    break
                                if reset_flag:
                        ...
                        self.thief.loc["letter", f"{i}{j}"] = letter
                        self.thief.loc["max", f"{i}{j}"] = max
                        self.thief.loc["index_app", f"{i}{j}"] = index
                        self.thief.loc["end", f"{i}{j}"] = end
