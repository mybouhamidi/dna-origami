...
class Sequence:
            ...
            for region in strand_structure:
                if region.name.islower():
                    if region.name in fixed_regions:
                        ...
                        region_defs[region.name.lower()] = "".join(
                            [
                                choice(list(Strand.allowed_bases))
    ...
    def mate(sequence1, sequence2):
        ...
        for region in sequence1.region_definitions:
            child_regions[region] = Sequence._mate_bases_crossover(
                sequence1.region_definitions[region],
                sequence2.region_definitions[region],
            )
    ...
    def build_strand(self, strand_structure):
        ...
        for region in strand_structure:
            if region.name.islower():
                ...
                bases += Strand.complement(
                    self.region_definitions[region.name.lower()]
                )[
                    ::-1
                ]  # Reversed, because strands only bind in the opposite direction
    ...
    def fitness(self, mfold, cache):
        ...
        if not region_hash in cache:
            strands = [
                self.build_strand(strand_structure)
...
class GeneticAlgorithm:
    ...
    def __init__(
        ...
        self.boltzmann_factor = 1
        """ (
            boltzmann_factor * 1000 / (TEMPERATURE * AVOGADRO * BOLTZMANN)
        ) """
        self.population = initial_sequences + [
            Sequence.random_sequence(structure, fixed_regions)
            for i in range(population_size - len(initial_sequences))
        ]
    ...
    def iterate(self):
        ...
        fitnesses = [
            sequence.fitness(self.mfold, self.cache) for sequence in self.population
        ]
        power_sum = sum(
            [exp(-fitness * self.boltzmann_factor) for fitness in fitnesses]
        )
        weighted_fitnesses = [
            exp(-fitness * self.boltzmann_factor) / power_sum for fitness in fitnesses
        ]
        ...
        self.population = [
            self.generate_child(weighted_fitnesses)
